...
func (a *api) processResponse(result model.Windy_Realtime_Report, icao string, headerID uuid.UUID) error {
	pmtsValues, pmtsStrings := buildValues(result, icao, headerID)
...
func buildWindyDetailSQL() string {
...
func buildValues(result model.Windy_Realtime_Report, icao string, headerID uuid.UUID) ([]model.RealtimeWeatherMapperToDB, string) {
	parameterizedValues := make([]model.RealtimeWeatherMapperToDB, 0, len(result.Ts))
	parameterizedStrings := buildParameters()
