...
//const repo.Timeout = time.Second * 50

type RealtimeForecastDBRepo struct {
	DB      *sql.DB
	Timeout time.Duration
}

func (repo *RealtimeForecastDBRepo) GetDBConnection() *sql.DB {
	return repo.DB
}

func (repo *RealtimeForecastDBRepo) GetWeatherStations(maxRecords int) (*[]model.WeatherForecast, error) {
	...
	rows, err := repo.DB.QueryContext(ctx, query, maxRecords)
	if err != nil {
		return nil, err
	}
	defer func() {
...
func (repo *RealtimeForecastDBRepo) InsertHourlyWeatherReport(sql string) (err error) {
	ctx, cancel := context.WithTimeout(context.Background(), repo.Timeout)
	defer cancel()

	stmtInserts, err := repo.DB.PrepareContext(ctx, sql)
	if err != nil {
		return err
	}
	defer func() {
...
func (repo *RealtimeForecastDBRepo) CheckRowExistence() (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), repo.Timeout)
	defer cancel()
	var count int
	err := repo.DB.QueryRowContext(ctx, "SELECT EXISTS(SELECT 1 FROM weather_stations LIMIT 1) as is_row_exists").Scan(&count)
	if err != nil {
		return 0, err
	}
	return count, nil
...
func (repo *RealtimeForecastDBRepo) InsertIntoDB(sql string) (err error) {
	ctx, cancel := context.WithTimeout(context.Background(), repo.Timeout)
	defer cancel()
	stmtInserts, err := repo.DB.PrepareContext(ctx, sql)
	if err != nil {
...
func (repo *RealtimeForecastDBRepo) CheckForTableExistence(dbName string, tableName string) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), repo.Timeout)
	defer cancel()
	var count int
	sql := "select count(table_name) AS count from information_schema.tables where table_schema=? and table_name=?"
	err := repo.DB.QueryRowContext(ctx, sql, dbName, tableName).Scan(&count)
	if err != nil {
		return 0, err
	}
	return count, nil
...
func (repo *RealtimeForecastDBRepo) InsertBatchStatements(sql string, pmtrsQuery string, pmtrsValues []model.RealtimeWeatherMapperToDB) (batchError error) {
	var execErr error
	ctx, cancel := context.WithTimeout(context.Background(), repo.Timeout)
	defer cancel()
	tx, execErr := repo.DB.BeginTx(ctx, nil)
	if execErr != nil {
		return execErr
	}
	defer func() {
